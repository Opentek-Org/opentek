let domain = [
  ...
  let htmld = `<select class="custom-select mr-sm-2 " id="type">`;
  htmld += `<option>Choose your Language</option>`;
  for (let index = 0; index < lang.length; index++) {
    htmld += `<option value="${index}">${lang[index]}</option>`;
  }
  
  htmld += "</select>";
  $("#selectOptionTwo").append(htmld);
